...
sub new_memcached {
    ...
    unless ($childpid) {
        ...
        my $cmd = "/usr/bin/env RUN_EINSTEIN=1 V=1 LOG_SUB_DIR=perltests ./serverctl restart $args";
...
sub DESTROY {
    #my $self = shift;
    #kill 2, $self->{pid};
    print STDERR "Destroying server...\n";
    system('./serverctl', 'stop');
    print STDERR "Done destroying server\n";
}
...
sub stop {
    ...
    system('./serverctl', 'stop');
    print STDERR "Done stopping server\n";
}

sub graceful_stop {
    #my $self = shift;
    #kill 'SIGUSR1', $self->{pid};
    print STDERR "Graceful stopping server...\n";
    system('./serverctl', 'stop');
    print STDERR "Done graceful stopping server\n";
}
