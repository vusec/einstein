...
static pid_t start_server(in_port_t *port_out, bool daemon, int timeout) {
    ...
    */
	...
	argv[arg++] = "/usr/bin/env";
	argv[arg++] = "RUN_EINSTEIN=1";
	argv[arg++] = "USE_LOG_DIR=1";
	argv[arg++] = "V=1";
	argv[arg++] = "LOG_SUB_DIR=testapp";
	argv[arg++] = "./serverctl";
	argv[arg++] = "restart";
        ...
        fprintf(stdout, "%s:%d: Executing: ", __FILE__, __LINE__);
        for(int i = 0; argv[i] != NULL; i++) { fprintf(stdout, "%s ", argv[i]); }
        fprintf(stdout, "\n");
        fflush(stdout);
    ...
    useconds_t wait_timeout = 1000000 * 20;
