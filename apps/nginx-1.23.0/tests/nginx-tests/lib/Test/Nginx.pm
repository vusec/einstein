...
sub run(;$) {
	...
	#print(STDERR "Waiting for nginx to start...\n");
...
sub waitforfile($;$) {
	...
	# wait for file to appear: check every 0.1 seconds for 20 seconds
...
sub read_file($) {
	my ($self, $name) = @_;
	local $/;
	my $filename = $self->{_testdir} . '/' . $name;

	$self->waitforfile($filename, 0) # pid=0  so that it's only checking whether filename exists, not whether pid exited
